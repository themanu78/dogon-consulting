...
const customersDisplay = Object.keys(custIcons).map((value, index, array) => {
  return (
    <Col key={index}>
      <img
        src={import.meta.env.VITE_PUBLIC_URL + `${custIcons[index].src}`}
        alt={import.meta.env.VITE_PUBLIC_URL + `${custIcons[index].name}`}
        className="clients"
        width="100rem"
        loading="lazy"
      />
